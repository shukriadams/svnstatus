...
rm -rf .clone &&
git clone --depth 1 --branch $TAG https://github.com/shukriadams/svnstatus.git .clone &&
...
docker build -t shukriadams/svnstatus . &&
docker tag shukriadams/svnstatus:latest shukriadams/svnstatus:$TAG &&
docker push shukriadams/svnstatus:latest &&
docker push shukriadams/svnstatus:$TAG &&
